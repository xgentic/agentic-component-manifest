...
import { resolveSettings, type CliArgs, type CliOverrides } from "./config.js";
import { runWatch } from "./watch.js";
import { BUILTIN_FRAMEWORKS, type AnalyzerSettings, type BuiltinFramework } from "./types.js";
...
export function parseCliArgs(argv: string[]): CliArgs {
  ...
  if (values.framework !== undefined) overrides.framework = validateFramework(values.framework);
...
function validateFramework(value: string): BuiltinFramework {
  if ((BUILTIN_FRAMEWORKS as readonly string[]).includes(value)) return value as BuiltinFramework;
  throw new UsageError(
    `unknown framework "${value}"; supported: ${BUILTIN_FRAMEWORKS.join(", ")} (omit for vanilla)`,
  );
}
...
function printOutcome(settings: AnalyzerSettings, outcome: AnalyzeOutcome): void {
  ...
  if (!settings.quiet && outcome.wrote) {
    process.stderr.write(`acm-analyzer: wrote ${settings.outdir}/agentic-component-manifest.json\n`);
  }
}

/** Verbose (`--dev`) startup summary of the resolved configuration. */
function logResolved(settings: AnalyzerSettings): void {
  const from = settings.config ? `config ${settings.config}` : "defaults + flags";
  process.stderr.write(
    `acm-analyzer: framework=${settings.framework ?? "vanilla"} outdir=${settings.outdir} ` +
